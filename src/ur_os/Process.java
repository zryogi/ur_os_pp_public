...
public class Process implements Comparable<Process> {
    ...
    private long vruntime; // Virtual runtime for CFS
    private int weight; // Weight for CFS (based on nice value/priority)
    ...
    public int compareTo(Process other) {
        long diff = this.vruntime - other.vruntime;
        ...
        // If vruntime is equal, use PID as tiebreaker for consistent ordering
    ...
    public void addVruntime(long deltaVruntime) {
        this.vruntime += deltaVruntime;
    }
    ...
    public void calculateWeight() {
        int[] weightTable = { 6,5,4,3,2,1,0 };

        int clampedPriority = Math.max(0, Math.min(6, this.priority));
        this.weight = weightTable[clampedPriority];
    }
}
